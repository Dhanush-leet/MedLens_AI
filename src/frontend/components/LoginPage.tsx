...
import { ShieldCheck, HeartPulse, Mail, Lock, ChevronRight } from 'lucide-react';
...
export const LoginPage: React.FC<Props> = ({ onSwitchToSignup, onBackToLanding }) => {
  ...
  return (
    <div id="patient-login-view" className="min-h-screen bg-[#FAF9F6] flex items-center justify-center p-4">
      <div className="w-full max-w-5xl bg-white border border-slate-200/80 rounded-3xl shadow-sm overflow-hidden grid grid-cols-1 md:grid-cols-12">
        ...
        <div className="md:col-span-5 bg-[#0E0E0E] text-[#FAF9F6] p-8 flex flex-col justify-between relative overflow-hidden min-h-[400px]">
          ...
          <div className="absolute top-[35%] right-[-30px] w-56 h-56 opacity-10 pointer-events-none md:opacity-20 animate-pulse">
            <div className="w-full h-full rounded-full border-4 border-[#E0362F] opacity-20 absolute scale-75 animate-ping" />
            <div className="w-full h-full rounded-full border border-slate-700 flex items-center justify-center">
              <HeartPulse className="w-20 h-20 text-[#E0362F]" />
            </div>
          </div>
        ...
        <div className="md:col-span-7 p-8 md:p-12 flex flex-col justify-center bg-[#FAF9F6]/40">
          <div className="space-y-2 mb-8">
            <span className="text-xs font-bold text-[#E0362F] uppercase tracking-wider">
              {t('patientPortalGateway')}
            </span>
            <h3 className="text-xl font-black text-slate-900 tracking-tight">
              Sign In to Your Account
            </h3>
            <p className="text-xs text-slate-500 font-semibold">
              Enter your clinical credentials to access the consultation portal.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              ...
              className="w-full py-3 bg-slate-950 hover:bg-[#E0362F] text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all shadow-sm hover:shadow-md flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
            >
              {isSubmitting ? (
